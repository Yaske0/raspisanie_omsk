...
Правила (согласованы с пользователем):
  ...
  • задержанные с НЕизвестным новым временем — показываем со словом ЗАДЕРЖАН,
    даже если их плановое время было в прошлую смену (рейс ещё летит);
  • ОТМЕНЁННЫЕ рейсы (статус «ОТМЕНЕН» на сайте) в расписание не попадают вообще.
...
def is_cancelled(f: Flight) -> bool:
    """Рейс отменён (на сайте выделяется красным, статус «ОТМЕНЕН»)."""
    return "ОТМЕН" in _status_upper(f)
...
def in_shift(f: Flight, shift: Shift) -> bool:
    # отменённые рейсы в расписание не попадают вообще
    if is_cancelled(f):
        return False
