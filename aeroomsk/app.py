...
PLANE_EASTER_EGG_CHANCE = 0.08        # ~8% — редкая пасхалка
...
class App(tk.Tk):
    ...
    # --- пасхалка: самолётик, изредка пролетающий над динозавром ----------
    def _maybe_spawn_plane(self):
        if not self._plane_active and random.random() < PLANE_EASTER_EGG_CHANCE:
            self._plane_active = True
            self._plane_x = -80.0

    def _plane_draw(self, x):
        self.bg.delete("plane")
        y = self.DINO_BASE - 34   # середина зоны анимации (между кнопками и журналом)
        t = "plane"
        # след
        for i, dx in enumerate((34, 24, 14)):
            self._cline(x - dx - 10, y + 2, x - dx, y + 2,
                        fill=TRAIL, width=2, tags=t, capstyle="round")
        # фюзеляж
        self._cpoly(x - 22, y, x + 18, y - 4, x + 26, y, x + 18, y + 4,
                    smooth=True, fill=PLANE, outline=PLANE_LINE, width=1, tags=t)
        # крылья
        self._cpoly(x - 2, y, x - 12, y - 12, x + 2, y - 2,
                    fill=PLANE, outline=PLANE_LINE, width=1, tags=t)
        self._cpoly(x - 2, y, x - 12, y + 12, x + 2, y + 2,
                    fill=PLANE, outline=PLANE_LINE, width=1, tags=t)
        # хвост
        self._cpoly(x - 18, y, x - 26, y - 8, x - 20, y, fill=PLANE,
                    outline=PLANE_LINE, width=1, tags=t)

    def _plane_tick(self):
        if not self._plane_active:
            return
        self._plane_x += 9
        if self._plane_x > W + 60:
            self._plane_active = False
            self.bg.delete("plane")
            return
        self._plane_draw(self._plane_x)

    # ------------------------------------------------------------------

    def _dino_start(self):
        self._maybe_spawn_plane()
