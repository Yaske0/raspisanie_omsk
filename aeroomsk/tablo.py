...
# Загрузка
# --------------------------------------------------------------------------
def detect_site_today(flights: list[Flight]) -> Optional[dt.date]:
    """
    Определить, какую дату сайт считает «сегодня», по факту скачанного
    табло THIS (сегодня): берём самую частую дату среди строк.

    Нужно, чтобы не зависеть от часового пояса/часов компьютера — сайт
    считает «сегодня» по своему времени (МСК), а ПК может быть настроен
    на другой часовой пояс. Если довериться только часам ПК, при
    расхождении в несколько часов возле полуночи программа попросит не ту
    дату (например, «завтра» вместо «сегодня»), и получит совсем не те
    рейсы, которые ожидала — молча.
    """
    if not flights:
        return None
